...
run_id=atomic_snapshot_$(date -u +%Y%m%dT%H%M%SZ)_$$
...
timeout 600 "$root/venvs/libero-eval-py310/bin/python" -u "$root/workspace/policy-relevant-imagined-state-repair/implementation/atomic_snapshot_probe.py" --out "$root/results/wam2repair/$run_id"
