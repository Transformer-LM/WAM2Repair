...
run_id=weight_vae_probe_$(date -u +%Y%m%dT%H%M%SZ)
...
mkdir -p "$TMPDIR"
"$root/conda/envs/fastwam-py311/bin/python" -u "$root/workspace/policy-relevant-imagined-state-repair/implementation/wam_weight_vae_probe.py" --out "$root/results/wam2repair/$run_id"
